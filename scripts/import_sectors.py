...
import csv
from workflow.models import Sector, SectorRelated
            ...
            for column in row:
                if column_num == 1:
                    ...
                    try:
                        ...
                        new_sector = Sector(sector=column)
                        new_sector.save()
                    print column
                if column_num == 2:
                    print "Sector="
        ...
        for row in sector2:
            ...
            for column in row:
                ...
                try:
                    ...
                    new_sector = Sector(sector=sector)
                    new_sector.save()
                    getSector = Sector.objects.get(sector=sector)
                ...
                try:
                    ...
                    new_sector = Sector(sector=sector_related)
                    new_sector.save()
                    getSubSector = Sector.objects.get(sector=sector_related)

                # add sector and sub-sector to the related object
                try:
                    checksub = SectorRelated.objects.filter(sector=getSector, sector_related=getSubSector).get()
                except SectorRelated.DoesNotExist:
                    print "adding new sector related"
                    print getSector
                    print getSubSector
                    print "or not"
                    new_related = SectorRelated(sector=getSector, sector_related=getSubSector)
                    new_related.save()
                print row
