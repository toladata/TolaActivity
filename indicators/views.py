...
import datetime
from dateutil.relativedelta import relativedelta #('%Y-%m-%d')
...
_PERIODICTARGET_DEFINITION = {
    ...
    Indicator.ANNUAL: lambda **params: {"period": "Year %s" % params.get('i'), \
        "start_date": (params.get('s') + relativedelta(years=+( (params.get('i') -1 )*1))).strftime('%b %d, %Y'), \
        "end_date": ( params.get('s') + relativedelta(years=+(params.get('i')*1)) + relativedelta(days=-1) ).strftime('%b %d, %Y')},
    Indicator.SEMI_ANNUAL: lambda **params: {"period": "Semi-annual period %s" % params.get('i'), \
        "start_date": (params.get('s') + relativedelta(months=+( (params.get('i') -1 )*6))).strftime('%b %d, %Y'), \
        "end_date": ( params.get('s') + relativedelta(months=+( params.get('i')*6)) + relativedelta(days=-1) ).strftime('%b %d, %Y')},
    Indicator.TRI_ANNUAL: lambda **params: {"period": "Tri-annual period %s" % params.get('i'), \
        "start_date": (params.get('s') + relativedelta(months=+( (params.get('i') -1 )*4))).strftime('%b %d, %Y'), \
        "end_date": ( (params.get('s') + relativedelta(months=+( params.get('i' )*4))) + relativedelta(days=-1) ).strftime('%b %d, %Y')},
    Indicator.QUARTERLY: lambda **params: {"period": "Quarter %s" % params.get('i'), \
        "start_date": (params.get('s') + relativedelta(months=+( (params.get('i') -1 )*3))).strftime('%b %d, %Y'), \
        "end_date": ( (params.get('s') + relativedelta(months=+( params.get('i')*3)))+ relativedelta(days=-1) ).strftime('%b %d, %Y')},
    Indicator.MONTHLY: lambda **params: {"period": (params.get('s') + relativedelta(months=+( (params.get('i') -1 )*1))).strftime("%B") \
        ...
        "start_date": (params.get('s') + relativedelta(months=+( (params.get('i') -1 )*1))).strftime('%b %d, %Y'), \
        "end_date": ( (params.get('s') + relativedelta(months=+params.get('i' ))) + relativedelta(days=-1) ).strftime('%b %d, %Y')},
}
